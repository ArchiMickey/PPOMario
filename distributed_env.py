...
def make_mario(world: int = 1, stage: int = 1, action_space: str = 'simple'):
    def _init():
        ...
        # env = SubprocVecEnv([(lambda: env) for _ in range(num_process)])
        
        return env
    return _init

def main():
    env = SubprocVecEnv([make_mario() for _ in range(6)])
    env.reset()
    done = [False for _ in range(6)]
    while True:
        env.render()
        action = [(env.action_space.sample()) for _ in range(6)]
